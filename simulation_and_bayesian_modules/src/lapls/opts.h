...
inline int GetOpts(int argc, char **argv, opts* opts_){
    ...
    if( argc < 3 || (argc == 2 && argv[1][0] != '-' && argv[1][1] != 'h') || argc == 1){
		EX_TRACE("[-i INPUT FILENAME][-o OUTPUT DIR][-s OUTPUT SIZE][-m 0~4]\n");
		return -1;
    }
    
    int ch;
    while ((ch = getopt_long(argc, argv, "hi:o:s:m:", longopts, NULL))!= -1) {
        switch (ch){
        case '?':
        ...
        case 'h':
            EX_TRACE("[-i INPUT FILENAME][-o OUTPUT DIR][-s OUTPUT SIZE]\n");
            return 0;
